...
- **Links freshness** (`sync-drive-metadata`): runs hourly, on the hour, across every client's `atl_links` rows — not gated on `cadence` being set. This job reads `modifiedTime`/`lastModifyingUser` via `files.get` and writes `drive_modified_at`/`drive_modified_by`, so "Last updated"/"By" populate for a link as soon as it's added. Each row can also have a `cadence` (weekly/fortnightly/monthly/quarterly/none); that only decides whether the row shows a current/due soon/overdue status versus "not tracked" — it no longer decides whether the row gets synced. Known limitation: for links pointing at a folder rather than a single file, `modifiedTime` only reflects changes to the folder itself (rename, description), not files added/edited inside it.
